...
var MongoUrl = "mongodb://" + config.user + ":" + config.password + "@"
...
var exp = {
	'register': function(req, res){
		if(req.body.email.length < 5 || req.body.password.length < 5){
			res.end(JSON.stringify({
				message: 'username and password . length need >= 5'
			}))
			return;
		}
				...
				sendEmail({
					...
					do_after_sent: function(){
						Db.db.collection('users').insert({
							...
							if(err){
								console.log(err);
								res.end(JSON.stringify({message: 'users query error'}))
								return;
							}
						});
						Db.db.collection('VER_KEY').insert({
							'email': req.body.email,
							'key': key
						},function(err, r){
							if(err){
								console.log(err);
								res.end(JSON.stringify({message: 'verkey query error'}))
								return;
							}
						});
						Db.db.collection('profiles').insert({
							email: req.body.email,
							nickname: req.body.nickname,
							AC: [],
							WA: []
							...
							if(err){
								console.log(err);
								res.end(JSON.stringify({message: 'profiles query error'}))
								return;
							}
						})
						res.end(JSON.stringify({
	...
	'login': function(req, res, is_login){
		...
		Db.db.collection('users').find({
			...
			if(!cnt){
				res.end(JSON.stringify({
					message: 'this user is not exist or password wrong'
				}));
				return;
			}else{
				var token = gen();
	...
	'verify': function(req, res){
		...
		var cursor = Db.db.collection('VER_KEY').find({
			'email': email,
			'key': key
		});
		cursor.toArray(function(err, docs){
			if(err){
				...
				if(docs.length != 1){
					res.end(JSON.stringify({
						message: "cant find this email||email>=2||!key"
					}));
					return;
				}else{
					var fd = Db.db.collection('users').find({
						'email': email
					});
					fd.toArray(function(err, cb){
						if(cb.length == 1){
							...
							ued.verified = true;
							Db.db.collection('users').updateOne(cb, ued);
	...
	'resend': function(req, res, is_login){
		...
		cursor.toArray(function(err, docs){
			...
			if(docs.length !== 1){
				res.end(JSON.stringify({
					message: 'you are verified or sth wrong'
				}))
				return;
			}
			if(is_login){
...
function gen(){
	var all = "0123456789qweasdZSzxcvbnfghrtyuiojklmpQAWXEDCFRVTGYBHNUJIMOKLP"
			.split("");
	var result = "";
	for(var i = 0; i < 20; i++){
